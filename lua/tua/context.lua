...
function M.get_context_limit(model_id)
  ...
  return MODEL_CONTEXT_LIMITS[model_id] or DEFAULT_LIMIT
end
...
function M.summarize(messages, instructions, provider_name)
  ...
  local summary_messages = {
    { role = "system", content = "You are a context summarizer. Condense the conversation history into a concise summary that preserves all key decisions, facts, and action items. Be specific about file paths, code changes, and tool results." },
    ...
    elseif type(msg.content) == "table" then
      ...
      for _, part in ipairs(msg.content) do
        ...
        elseif part.type == "tool_result" then
          table.insert(texts, "[Tool result: " .. (part.tool_use_id or "?") .. "] " .. tostring(part.content):sub(1, 200))
        end
      end
